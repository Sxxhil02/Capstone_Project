"""
Toy model: preference-sensitive boundary routing (outer network only)
-----------------------------------------------------------------------
Companion to "Preference-Sensitive Boundary Routing for Event Travel".

Scenario: attendees of the (fictional) International Conference for
Efficient Traffic are travelling from two hotel clusters, through a
small synthetic outer road network, towards one of several boundary
nodes that lead in to the World Traffic Organisation venue.
...
the whole thing can be shown on a single slider. Congestion is NOT
modelled here on purpose -- this script tests only the first half of
the research idea, namely how much a traveller's recommended entry
point moves as their own preference weights change. It is Pareto-exact
for this toy graph (multi-label DAG shortest paths / a simplified,
heuristic-free NAMOA*), not an approximation.

Controls
--------
- Slider at the bottom: time-weight w_time (fare-weight = 1 - w_time).
- Radio buttons: pick which traveller (origin) to inspect.
- The network panel highlights the recommended path in bold; the bar
  panel shows the scalarised cost to every boundary node, cheapest
  highlighted, with the resulting ranking printed as a title.
"""
...
# Hotel cluster A (top-left triangle) and cluster B (top-right triangle)
positions["A0"] = (1.0, 10.0)
positions["A1"] = (2.0, 10.7)
positions["A2"] = (1.5, 9.3)
positions["B0"] = (7.0, 10.0)
positions["B1"] = (8.0, 10.7)
positions["B2"] = (7.5, 9.3)
origins = ["A0", "A1", "A2", "B0", "B1", "B2"]

# Outer network, two intermediate layers
...
# Boundary nodes (candidate entry points to the venue)
boundary_nodes = [f"B{i}" for i in range(4)]
...
positions["WTO"] = (4.0, 0.8)

# Wire hotel clusters into the outer network
for o in ["A0", "A1", "A2"]:
...
# Outer layer 2 -> boundary nodes
for i, u in enumerate(layer2):
    add_edge(u, boundary_nodes[i], (5, 15), (1.0, 5.0))
    add_edge(u, boundary_nodes[(i + 1) % 4], (5, 15), (1.0, 5.0))

# Boundary -> venue, shown as dashed context edges (not costed / not routed over)
context_edges = [(b, "WTO") for b in boundary_nodes]
...
# Topological order: origins, then layer1, then layer2, then boundary
...
def draw_network(ax, source, w_time):
    ax.clear()
    ax.set_title("Outer network (toy)", fontsize=11)
    ax.set_xlim(-0.5, 8.5)
    ax.set_ylim(0, 11.5)
    ax.axis("off")
    ...
    for n, (x, y) in positions.items():
        if n == source:
            ...
            ax.annotate("traveller", (x, y), textcoords="offset points",
                        xytext=(0, 10), ha="center", fontsize=8)
        elif n == "WTO":
            ax.scatter([x], [y], s=200, marker="s", color="navy", zorder=3)
            ax.annotate("WTO (venue)", (x, y), textcoords="offset points",
...
def draw_bar(ax, source, w_time):
    ...
    rank_str = " > ".join(names)
    ax.set_title(
        f"Traveller at {source}   |   w_time={w_time:.2f}, w_fare={w_fare:.2f}\n"
        f"ranking (best\u2192worst): {rank_str}",
        fontsize=10,
    )
