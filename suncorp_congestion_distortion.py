"""
Toy model: congestion-naive vs. congestion-aware gate routing (Suncorp Stadium)
-----------------------------------------------------------------------
...
Idea
----
outer_routing_demo.py ranks Suncorp Stadium gates for an attendee using
only their own path attributes (time, fare) -- congestion at the gates
is ignored entirely, on the assumption that the outer network is what
matters and gate congestion can be dealt with separately.

This script tests how costly that simplification is. It:

  1. Routes a synthetic population of attendees "congestion-naively"
     (exactly as in outer_routing_demo.py) and tallies how many pick
     each gate -- this is the naive load.
  2. Turns that naive load into a simple congestion penalty g_j per
     gate (more people converging on a gate -> longer queue). This is
     a crude stand-in for a real gate/turnstile queueing model, not
     one -- see the write-up for the honest version of that caveat.
  3. Re-routes the same attendees "congestion-aware", i.e. using
     Ci_aware(j) = Ci(j) + w_time * g_j, and tallies the new load.
  4. For every attendee, computes the distortion of having followed
...
Controls
--------
- Slider: congestion severity k (minutes of extra queueing per
  attendee converging on a gate). k=0 recovers the naive-only case.
- The three panels update live: the congestion penalty per gate, how
  load redistributes from naive to aware routing, and the distribution
  of regret D_i across the population.
"""
...
def redraw(k):
    ...
    ax_load.set_title("Attendee load per gate\n(naive vs. congestion-aware)", fontsize=10)
    ...
    ax_hist.set_title(
        ...
        f"{changed_frac * 100:.0f}% of attendees redirected",
        fontsize=10,
    )
    ax_hist.set_xlabel("D_i")

    fig.suptitle(
        f"Suncorp Stadium concert \u2014 congestion severity k = {k:.2f} min/attendee"
